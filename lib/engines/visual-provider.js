...
// ═══════════════════════════════════════════
// FLUX.1-schnell via Hugging Face — 100% 무료
// ═══════════════════════════════════════════

export async function generateWithHuggingFace(prompt, options = {}) {
  const { aspectRatio = '9:16', overlayText = '' } = options;

  // Arsenal 태그 + 타이포그래피 강화
  let enhancedPrompt = `${prompt}, ${ARSENAL_TAGS}`;
  if (overlayText) {
    enhancedPrompt += `, with bold clean typography reading "${overlayText}", sharp text details`;
  }

  console.log(`🤗 [HUGGINGFACE] FLUX.1-schnell 생성 시작 — AR: ${aspectRatio}`);

  const apiUrl = 'https://api-inference.huggingface.co/models/black-forest-labs/FLUX.1-schnell';

  // HF 토큰 있으면 사용 (없어도 무료 추론 가능, 단 rate limit 더 제한적)
  ...
  if (!response.ok) {
    ...
    // 모델 로딩 중인 경우 (503) — 재시도 안내
    if (response.status === 503) {
      throw new Error('FLUX.1-schnell 모델 로딩 중 (콜드 스타트). 30초 후 다시 시도하세요.');
    }
    throw new Error(`HuggingFace API ${response.status}: ${errText.substring(0, 200)}`);
  }

  // HF는 이미지를 raw binary (PNG)로 반환
  const imageBuffer = Buffer.from(await response.arrayBuffer());
  const base64 = imageBuffer.toString('base64');
  const dataUrl = `data:image/png;base64,${base64}`;

  // 파일로도 저장 (대시보드 재접속 시 유지)
  const fs = await import('fs/promises');
  const path = await import('path');
  const renderDir = path.default.join(process.cwd(), 'public', 'renders');
  ...
  try {
    ...
    const filename = `hf_flux_${Date.now()}.png`;
    const filePath = path.default.join(renderDir, filename);
    await fs.writeFile(filePath, imageBuffer);
    console.log(`✅ [HUGGINGFACE] 생성 완료 — ${filename} (${(imageBuffer.length / 1024).toFixed(1)}KB)`);
    ...
    return {
      ...
      dataUrl,
      prompt: enhancedPrompt,
      metadata: { aspectRatio, model: 'FLUX.1-schnell', cost: 'FREE' },
    };
  } catch {
    // 파일 저장 실패 시 DataURL만 반환
    console.log(`✅ [HUGGINGFACE] 생성 완료 (DataURL 전용)`);
    return {
      provider: 'huggingface',
      imageUrl: dataUrl,
      prompt: enhancedPrompt,
...
// ═══════════════════════════════════════════
// FLUX.1 [pro] via Fal.ai — 포토리얼리즘 (유료)
// ═══════════════════════════════════════════
  ...
  return [
    {
      ...
      badge: '🆓 Zero-Cost',
      description: '100% 무료 — HuggingFace FLUX.1-schnell 포토리얼리즘',
      available: true, // 무료이므로 항상 사용 가능 (HF 토큰 있으면 더 빠름)
      bestFor: ['sns', 'poster', 'card'],
      cost: 'FREE',
    },
    {
      id: 'ideogram',
