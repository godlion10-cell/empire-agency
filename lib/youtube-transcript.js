...
export async function fetchTranscript(urlOrId) {
  ...
  // ──────────────────────────────
  // 🟡 Level 2.5: oEmbed 제목 기반 스마트 폴백
  // ──────────────────────────────
  let oEmbedTitle = null;
  try {
    console.log(`🟡 Level 2.5: oEmbed API로 영상 제목 추출...`);
    ...
    if (oembedRes.ok) {
      ...
      console.log(`✅ oEmbed 성공: "${oEmbedTitle}" (by ${authorName})`);
    }
  } catch (e) {
  ...
  // ──────────────────────────────
  try {
    console.log(`🟣 Level 3: Gemini 2.5 Flash 멀티모달 분석 가동...`);
    const segments = await geminiAnalyzeVideo(videoUrl);
    if (segments && segments.length > 0) {
      // 더미 세그먼트 감지: 1개짜리이고 텍스트에 '수동 확인' 등이 포함된 경우 실패로 간주
      const isDummy = segments.length === 1 && /수동 확인|분석 대기|추출 실패|전사 실패/.test(segments[0].text);
      if (isDummy) {
        console.log(`⚠️ Level 3 더미 결과 감지 — 실패 처리`);
      } else {
  ...
  // ──────────────────────────────
  // 🛡️ 최종 안전망: oEmbed 제목 활용 또는 기본 텍스트
  // ──────────────────────────────
  if (oEmbedTitle) {
    ...
    return formatResult(videoId, [
      ...
      { start: 8, end: 15, text: `이 영상의 제목을 바탕으로 AI가 숏폼 대본을 새롭게 창작합니다.` },
    ], 'oembed-title');
  }
