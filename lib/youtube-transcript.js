...
async function geminiAnalyzeVideo(videoUrl) {
  ...
  const prompt = `당신은 전문 영상 분석가입니다.

이 YouTube 영상의 오디오를 분석하여 자막 데이터를 JSON으로 생성하세요.

[지시사항]
1. 영상의 음성을 분석하여 타임코드별 자막을 생성하라.
2. 각 자막은 3~7초 단위로 끊어라.
3. 텍스트는 해요체로 자연스럽게 정제하라.
4. 핵심 정보 위주로 텍스트를 정리하라.
5. 반드시 JSON으로만 응답하라.

[출력 형식]
{
  "segments": [
    { "start": 0.0, "end": 3.5, "text": "자막 내용" },
    { "start": 3.5, "end": 7.0, "text": "다음 자막" }
  ]
}
...
영상 URL: ${videoUrl}`;
  ...
  try {
    ...
    console.log(`⚠️ Gemini 분석 에러: ${e.message}`);
  }
