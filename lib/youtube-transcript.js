 ...
 * Level 2.5: oEmbed API 영상 메타데이터 추출 (항상 실행, 후속 레벨에 제목 제공)
 * Level 3: 오디오 스트림 추출 → HuggingFace 무료 Whisper STT (OpenAI 유료 폴백)
 * Level 4: Gemini 2.5 Flash 멀티모달 — YouTube URL 직접 분석 (궁극 폴백)
 * Level 5: oEmbed 제목 기반 AI 창작 (최후의 보루, 무조건 성공)
 * 
 * ★ 파이프라인은 절대 실패하지 않습니다.
 */
...
export async function fetchTranscript(urlOrId) {
  ...
  // ──────────────────────────────
  // 🟡 Level 2.5: oEmbed 제목 추출 (항상 실행 — 후속 레벨에 제목 제공)
  // ──────────────────────────────
  let oEmbedTitle = null;
  let oEmbedAuthor = null;
  try {
    console.log(`🟡 Level 2.5: oEmbed API로 영상 메타데이터 추출...`);
    ...
    if (oembedRes.ok) {
      ...
      console.log(`✅ oEmbed 성공: "${oEmbedTitle}" (by ${oEmbedAuthor})`);
    }
  } catch (e) {
    console.log(`⚠️ Level 2.5 oEmbed 실패: ${e.message?.substring(0, 60)}`);
  }

  // ──────────────────────────────
  // 🔶 Level 3: Whisper STT (오디오 스트림 추출 → HuggingFace 무료 Whisper)
  // ──────────────────────────────
  try {
    const hfKey = process.env.HF_API_KEY || process.env.HUGGINGFACE_API_KEY;
    const openaiKey = process.env.OPENAI_API_KEY;
    const sttKey = hfKey || openaiKey;
    const sttProvider = hfKey ? 'huggingface' : 'openai';

    if (sttKey) {
      console.log(`🔶 Level 3: Whisper STT (${sttProvider}) — 오디오 스트림 추출 시도...`);
      const audioUrl = await extractAudioStreamUrl(videoId);
      if (audioUrl) {
        console.log(`🔶 Level 3: 오디오 URL 확보 — Whisper 전사 중...`);
        const audioRes = await fetch(audioUrl, {
          headers: { 'User-Agent': USER_AGENT, Range: 'bytes=0-26214400' }, // 25MB 제한
          signal: AbortSignal.timeout(30000),
        });
        if (audioRes.ok) {
          const audioBuffer = Buffer.from(await audioRes.arrayBuffer());
          if (audioBuffer.length > 1000) { // 최소 1KB
            let whisperText = null;

            if (sttProvider === 'huggingface') {
              // ═══ 🆓 HuggingFace 무료 Inference API ═══
              console.log(`  🧠 HuggingFace whisper-large-v3 호출 (${Math.round(audioBuffer.length / 1024)}KB)...`);
              const hfRes = await fetch(
                'https://api-inference.huggingface.co/models/openai/whisper-large-v3',
                {
                  ...
                  body: audioBuffer,
                  signal: AbortSignal.timeout(90000), // HF는 cold start가 길 수 있음
                }
              );

              if (hfRes.ok) {
                const hfData = await hfRes.json();
                whisperText = hfData.text?.trim();
                if (hfData.error) {
                  console.log(`  ⚠️ HF 응답 에러: ${hfData.error}`);
                  whisperText = null;
                }
              } else {
                const errText = await hfRes.text().catch(() => '');
                console.log(`  ⚠️ HF HTTP ${hfRes.status}: ${errText.substring(0, 100)}`);
              }
            } else {
              // ═══ 💰 OpenAI Whisper-1 (폴백) ═══
              console.log(`  🧠 OpenAI Whisper-1 호출 (${Math.round(audioBuffer.length / 1024)}KB)...`);
              ...
              if (oaiRes.ok) {
                ...
                whisperText = oaiData.text?.trim();
              }
            }

            if (whisperText && whisperText.length > 20) {
              console.log(`✅ Level 3 Whisper (${sttProvider}) 성공: ${whisperText.length}자`);
              const sentences = whisperText.split(/[.!?。？！\n]+/).filter(s => s.trim());
              const segments = sentences.map((sentence, i) => ({
                start: i * 5,
                end: (i + 1) * 5,
                text: sentence.trim(),
              })).filter(s => s.text);
              return formatResult(videoId, segments, `whisper-${sttProvider}`);
            }
          }
        }
      }
    } else {
      console.log(`⚠️ Level 3 건너뜀: HF_API_KEY / OPENAI_API_KEY 모두 미설정`);
    }
  } catch (e) {
    console.log(`⚠️ Level 3 Whisper 실패: ${e.message?.substring(0, 80)}`);
  }

  // ──────────────────────────────
  // 🟣 Level 4: Gemini 멀티모달 (궁극 폴백)
  // ──────────────────────────────
  try {
    console.log(`🟣 Level 4: Gemini 2.5 Flash 멀티모달 분석 가동...${oEmbedTitle ? ` (제목: "${oEmbedTitle}")` : ''}`);
    const segments = await geminiAnalyzeVideo(videoUrl);
    if (segments && segments.length > 0) {
      const isDummy = segments.length === 1 && /수동 확인|분석 대기|추출 실패|전사 실패/.test(segments[0].text);
      if (isDummy) {
        console.log(`⚠️ Level 4 더미 결과 감지 — 실패 처리`);
      } else {
        console.log(`✅ Level 4 성공: ${segments.length}개 세그먼트 (AI 생성)`);
        return formatResult(videoId, segments, 'gemini');
      }
    }
  } catch (e) {
    console.log(`⚠️ Level 4 실패: ${e.message?.substring(0, 120)}`);
  }

  // ──────────────────────────────
  // 🛡️ 최종 안전망: oEmbed 제목 기반 창작 또는 기본 텍스트
  // ──────────────────────────────
  if (oEmbedTitle) {
    console.log(`🛡️ 모든 레벨 실패 — oEmbed 제목 기반 결과 반환: "${oEmbedTitle}"`);
    return formatResult(videoId, [
      { start: 0, end: 3, text: `[자막 추출 불가 — 제목 기반 분석 모드]` },
      { start: 3, end: 8, text: `영상 제목: ${oEmbedTitle}` },
      { start: 8, end: 13, text: `채널: ${oEmbedAuthor || '알 수 없음'}` },
      { start: 13, end: 20, text: `이 영상의 제목을 바탕으로 AI가 숏폼 대본을 새롭게 창작합니다.` },
    ], 'oembed-title');
  }
...
// ═══════════════════════════════════════
// Level 3: Whisper용 오디오 스트림 URL 추출
// ═══════════════════════════════════════

/**
 * YouTube 페이지에서 오디오 전용 스트림 URL을 추출
 * adaptiveFormats에서 audio/webm 또는 audio/mp4를 찾음
 */
async function extractAudioStreamUrl(videoId) {
  try {
    const resp = await fetch(`https://www.youtube.com/watch?v=${videoId}`, {
      headers: { 'User-Agent': USER_AGENT, 'Accept-Language': 'ko-KR,ko;q=0.9,en;q=0.8' },
    ...
    for (let i = jsonStart; i < html.length; i++) {
      if (html[i] === '{') depth++;
        ...
        if (depth === 0) {
          ...
          // audio-only 포맷 중 URL이 직접 노출된 것 찾기 (sig cipher 필요 없는 것)
          const audioFormats = formats
            .filter(f => f.mimeType?.startsWith('audio/') && f.url && !f.signatureCipher)
            .sort((a, b) => (b.bitrate || 0) - (a.bitrate || 0)); // 높은 비트레이트 우선

          if (audioFormats.length > 0) {
            console.log(`  🔶 오디오 포맷 발견: ${audioFormats[0].mimeType} (${Math.round(audioFormats[0].contentLength / 1024 / 1024)}MB)`);
            return audioFormats[0].url;
          }

          console.log(`  ⚠️ 오디오 포맷 없음 (signatureCipher 보호됨)`);
          return null;
        }
      }
    }
    ...
    console.log(`  ⚠️ 오디오 스트림 추출 에러: ${e.message?.substring(0, 60)}`);
    return null;
  }
}

// ═══════════════════════════════════════
// Level 4: Gemini 멀티모달 분석
// ═══════════════════════════════════════
...
영상 URL: ${videoUrl}`;
  ...
  try {
    ...
    // Gemini가 빈 결과를 반환한 경우
    throw new Error('Gemini가 유효한 자막 세그먼트를 반환하지 않았습니다.');
  } catch (e) {
    console.error(`⚠️ Gemini URL 분석 에러: ${e.message?.substring(0, 200)}`);
    // ★ 더미 텍스트를 반환하지 않고 에러를 그대로 throw
    throw new Error(`Gemini STT 실패: ${e.message?.includes('API key') ? 'API 키 만료 또는 무효 — Google AI Studio에서 갱신 필요' : e.message?.substring(0, 100)}`);
  }
}
