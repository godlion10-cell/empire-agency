 ...
 * Level 3: 오디오 스트림 추출 → OpenAI Whisper-1 STT (고정밀, API 크레딧 소모)
...
export async function fetchTranscript(urlOrId) {
  ...
  // ──────────────────────────────
  // 🔶 Level 3: Whisper STT (오디오 스트림 추출 → OpenAI Whisper-1)
  // ──────────────────────────────
  try {
    const openaiKey = process.env.OPENAI_API_KEY;
    if (openaiKey) {
      console.log(`🔶 Level 3: Whisper STT — 오디오 스트림 추출 시도...`);
      const audioUrl = await extractAudioStreamUrl(videoId);
      if (audioUrl) {
        console.log(`🔶 Level 3: 오디오 URL 확보 — Whisper-1 전사 중...`);
          ...
          if (audioBuffer.length > 1000) { // 최소 1KB
            ...
            if (whisperRes.ok) {
              ...
              if (text && text.length > 20) {
                console.log(`✅ Level 3 Whisper 성공: ${text.length}자`);
                // Whisper는 단일 텍스트 → 5초 단위로 분할
                const words = text.split(/[.!?。？！\n]+/).filter(s => s.trim());
                const segments = words.map((sentence, i) => ({
                  start: i * 5,
                  end: (i + 1) * 5,
                  text: sentence.trim(),
                })).filter(s => s.text);
                return formatResult(videoId, segments, 'whisper');
              }
            }
          }
        }
      ...
      console.log(`⚠️ Level 3 건너뜀: OPENAI_API_KEY 미설정`);
    }
  } catch (e) {
